...
if __name__ == "__main__":
    ...
    while 1:
        ...
        while 1:
            ...
            fprimeprime = np.mat([[0,0],[0,0]])                                                                           ###YOUR CODE HERE###

            #compute the first and second derivatives from each hyperplane and aggregate
            for j in range(0,numplanes):
                #fprime_for_plane_j = np.mat([[(a.item(0,j))/( np.dot(a[:,j].T, x).item(0)-b.item(j))],[(a.item(1,j))/(np.dot(a[:,j].T,x).item(0)-b.item(j))]])                  ###YOUR CODE HERE###

                #fprimeprime_for_plane_j = np.mat([[(-(a.item(0,j))**2)/((np.dot(a[:,j].T, x).item(0)-b.item(j))**2), (-(a.item(0,j))**2)/((np.dot(a[:,j].T, x).item(0)-b.item(j))**2)], [(-(a.item(1,j))**2)/((np.dot(a[:,j].T, x).item(0)-b.item(j))**2), (-(a.item(1,j))**2)/((np.dot(a[:,j].T, x).item(0)-b.item(j))**2)]])       ###YOUR CODE HERE###

                fprime_for_plane_j = (1/ (-b.item(j)+np.dot(a[:,j].T,x).item(0)))*a[:,j]
                fprimeprime_for_plane_j = (1/ (-b.item(j)+(np.dot(a[:,j].T,x)).item(0))**2)*(np.dot(a[:,j], a[:,j].T))
            ...
            if((lambda2.item(0)/2) <=newton_epsilon):                                                                           ###YOUR CODE HERE###)
                break;
            ...
            while 1:
                ...
                if(pastboundary or (fnew > f + alpha*k*fprime.T*step)):                                                 ###YOUR CODE HERE###):  #put in the check for terminating backtracking line search
                                                                                         ...
                                                                                         ###YOUR CODE HERE###
		    #print 'fnew: %f\n'%(fnew)
		    #print 'RHS: %f\n'%(f + alpha*k*fprime.T*step)
		    #print 'pastboundary: %f\n'%(pastboundary)
		    #print 'k: %f\n'%(k)
                    k = beta*k 
                    #quitter = quitter + 1
                else:
